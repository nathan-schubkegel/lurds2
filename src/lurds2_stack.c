...
void* Stack_Peek(Stack stack)
{
  ...
  if (data->count == 0)
  {
    DIAGNOSTIC_STACK_ERROR("cannot peek empty stack");
    return 0;
  }
...
void* Stack_Get(Stack stack, int32_t index)
{
  StackData* data = (StackData*)stack;
  if (data == 0)
  {
  ...
  {
    DIAGNOSTIC_STACK_ERROR("invalid index argument");
    return 0;
  }

  return data->elements + data->elementSize * index;
}
